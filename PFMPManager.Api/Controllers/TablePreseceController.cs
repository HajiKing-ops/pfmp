...
namespace PFMPManager.Api.Controllers
{
    ...
    public class TablePreseceController : ControllerBase
    {
        ...
        public async Task<IActionResult> Presence()
        {
            ...
            var todayPresences = await _context.TablePresence
                .Where(p =>
                    studentIds.Contains(p.Id_Utilisateur) &&
                    p.DateJour.HasValue &&
                    p.DateJour.Value.Date == todayDate)
                .ToListAsync();

            var updatedCount = 0;

            foreach (var presence in todayPresences)
            {
                if (presence.Etat != null &&
                    presence.Etat.Trim().Equals("NON_RENSEIGNE", StringComparison.OrdinalIgnoreCase))
                {
                    presence.Etat = Present;
                    presence.Retard = 0;
                    presence.Justification = false;
                    updatedCount++;
                }
            }

            var existingStudentIds = todayPresences
                .Select(p => p.Id_Utilisateur)
                .ToHashSet();

            var newPresences = studentIds
                .Where(studentId => !existingStudentIds.Contains(studentId))
                .Select(studentId => new TablePresence
                {
            ...
            if (updatedCount == 0 && newPresences.Count == 0)
            {
