...
namespace PFMPManager.Api.Controllers
{
    ...
    public class PfmpController : ControllerBase
    {
        ...
        public async Task<IActionResult> CompletePfmp(CreateCompletePfmpDto request)
        {
           ...
           var currentStudentId = currentUserResult.UserId;
            ...
            try
            {
                ...
                await CreatePresenceRowsForPfmpAsync(currentStudentId, request.DateDebut.Value, request.DateFin.Value, validPlanningDays);
        ...
        private async Task CreatePresenceRowsForPfmpAsync(
            int studentId,
            DateTime dateDebut,
            DateTime dateFin,
            List<CreatePlanningJoursDto> planningJours)
        {
            var joursTravail = planningJours
                .Select(j => j.Jour?.Trim())
                .Where(j => !string.IsNullOrWhiteSpace(j))
                .ToHashSet(StringComparer.OrdinalIgnoreCase);

            var presences = new List<TablePresence>();

            for (var date = dateDebut.Date; date <= dateFin.Date; date = date.AddDays(1))
            {
                var jourFrancais = date.DayOfWeek switch
                {
                ...
                if (!joursTravail.Contains(jourFrancais))
                {
                    continue;
                }

                presences.Add(new TablePresence
                {
            ...
            _context.TablePresence.AddRange(presences);
            await _context.SaveChangesAsync();
        }
